...
class YamlUnicodeReaderTest {
    ...
    void noBom() throws IOException {
        var input = new ByteArrayInputStream("1".getBytes());
        var reader = new YamlUnicodeReader(input);
        reader.init();
    ...
    void utf8() throws IOException {
        var input = new ByteArrayInputStream(new byte[] {(byte) 0xEF, (byte) 0xBB, (byte) 0xBF, 49});
        var reader = new YamlUnicodeReader(input);
        reader.init();
        assertEquals(StandardCharsets.UTF_8, reader.getEncoding(), "no BOM must be detected as UTF-8");
    ...
    void feff32() throws IOException {
        var input = new ByteArrayInputStream(new byte[] {0x00, 0x00, (byte) 0xFE, (byte) 0xFF, 0, 0, 0, 49});
        var reader = new YamlUnicodeReader(input);
        reader.init();
    ...
    void fffe32() throws IOException {
        var input = new ByteArrayInputStream(new byte[] {(byte) 0xFF, (byte) 0xFE, 0x00, 0x00, 49, 0, 0, 0});
        var reader = new YamlUnicodeReader(input);
        reader.init();
    ...
    void feff16() throws IOException {
        var input = new ByteArrayInputStream(new byte[] {(byte) 0xFE, (byte) 0xFF, 0, 49});
        var reader = new YamlUnicodeReader(input);
        reader.init();
    ...
    void fffe16() throws IOException {
        var input = new ByteArrayInputStream(new byte[] {(byte) 0xFF, (byte) 0xFE, 49, 0});
        var reader = new YamlUnicodeReader(input);
        reader.init();
