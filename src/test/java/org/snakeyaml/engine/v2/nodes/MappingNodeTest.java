...
class MappingNodeTest {
    ...
    @Test
    void testToString() {
        var tuple1 = new NodeTuple(
            new ScalarNode(Tag.STR, "a", ScalarStyle.PLAIN),
            new ScalarNode(Tag.INT, "1", ScalarStyle.PLAIN)
        );
        var list = new ArrayList<NodeTuple>();
        list.add(tuple1);
        var mapping = new MappingNode(Tag.MAP, list, FlowStyle.FLOW);
        var tuple2 = new NodeTuple(new ScalarNode(Tag.STR, "self", ScalarStyle.PLAIN), mapping);
        list.add(tuple2);
